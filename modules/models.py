...
class FPN(tf.keras.layers.Layer):
    ...
    def call(self, x):
        ...
        #up3 = tf.image.resize_images(output3, [up_h, up_w], method=1)
        output2 = output2 + up3
        output2 = self.merge2(output2)

        up_h, up_w = tf.shape(output1)[1], tf.shape(output1)[2]
        up2 = tf.image.resize(output2, [up_h, up_w], method='nearest')
        #up2 = tf.image.resize_images(output2, [up_h, up_w], method=1)
        output1 = output1 + up2
...
def RetinaFaceModel(cfg, training=False, iou_th=0.1, score_th=0.5,
