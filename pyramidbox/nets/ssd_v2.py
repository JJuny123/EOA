...
class PyramidBoxModel():
    ...
    def __additional_ssd_block(self, end_points,channels, net, is_training=False):
        ...
        logits, localisations,net_for_feature_view = self.ssd_multibox_layer(end_points,
                                  channels,
                                  self.feat_layers,
                                  self.normalizations,
        ...
        if is_training==True:
            ...
            return predictions, localisations, logits, end_points, net_for_feature_view
    ...
    def ssd_multibox_layer(self, end_points,
        ...
        for i, layer in enumerate(feat_layers):
            with tf.variable_scope(layer + '_box'):
                ...
                # #下面一条语句增加于818为了测试在cpm模块前的特征值
                # print("特征tensor:",net)
                net_for_feature_view.append(net)
                net=self.cpm(net)
                # Location.
                num_loc_pred =  4
        ...
        return logits,localisations,net_for_feature_view
